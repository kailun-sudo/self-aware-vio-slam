...
V2 dataset builder with two data sources:

1. sweep_runs
   Preferred. Builds the dataset from real baseline + degraded replay runs
   generated by the multisequence degradation sweep.

2. sequence_dirs
   Fallback. Builds the dataset from packaged per-sequence directories under
...
def _split_run_records(records: List[Dict[str, object]], config: dict) -> Dict[str, List[Dict[str, object]]]:
    ...
    for sequence, seq_group in frame.groupby('sequence', sort=True):
        ...
        for severity, severity_group in degraded_group.groupby('severity', sort=True):
            run_ids = severity_group['run_id'].tolist()
            order = rng.permutation(len(run_ids))
            shuffled = [run_ids[idx] for idx in order]
            n_train, n_val, n_test = _compute_split_counts(
                len(shuffled), train_ratio=train_ratio, val_ratio=val_ratio, test_ratio=test_ratio
            )
...
def _resolve_data_source(config: dict) -> Tuple[str, List[Dict[str, object]]]:
    dataset_cfg = config['dataset']
    ...
    if source_mode == 'sweep_runs':
        ...
        print(f"Using sweep run source: {sweep_results_path}")
        return 'sweep_runs', sweep_records

    if source_mode == 'sequence_dirs':
        if not sequence_records:
            raise FileNotFoundError(f"No sequence data found in {data_dir}")
        print(f"Using sequence directory source: {data_dir}")
        return 'sequence_dirs', sequence_records
    ...
    if source_mode == 'hybrid' or (source_mode == 'auto' and sequence_records and sweep_records):
        ...
        print(f"Using hybrid source: sequence dirs from {data_dir} + degraded sweep runs from {sweep_results_path}")
        return 'hybrid', hybrid_records
...
def build_dataset(config: dict | None = None) -> Dict:
    ...
    source_mode, records = _resolve_data_source(config)
